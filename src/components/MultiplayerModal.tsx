...
export function MultiplayerModal({ triggerClassName, triggerLabel }: Props) {
  ...
  const modalOverlay = (
    <div className="fixed inset-0 z-[99999] flex items-center justify-center p-4 bg-black/80 backdrop-blur-md animate-in fade-in duration-200">
      <div className="relative w-full max-w-md bg-card text-card-foreground border border-border/70 rounded-3xl p-6 sm:p-8 shadow-2xl space-y-6 animate-in zoom-in-95 duration-200 max-h-[90vh] overflow-y-auto">
        {/* Close Button */}
        <button
          type="button"
        ...
        <div className="text-center space-y-2 pt-2">
          <div className="w-12 h-12 rounded-2xl bg-amber-500/10 border border-amber-500/30 text-amber-500 flex items-center justify-center mx-auto shadow-xs">
  ...
  return (
    <>
      ...
      {isOpen && mounted && createPortal(modalOverlay, document.body)}
    </>
  );
}

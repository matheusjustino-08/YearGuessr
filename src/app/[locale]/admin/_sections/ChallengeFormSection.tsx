...
const DIFFICULTIES = [
  { id: 'facil', label: 'Fácil', desc: 'Evento icônico e mundialmente conhecido' },
  { id: 'normal', label: 'Normal', desc: 'Exige raciocínio e contexto histórico' },
  { id: 'dificil', label: 'Difícil', desc: 'Evento obscuro, imagem ambígua' },
];
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-6">
        ...
        {[
          ...
          <div key={lang} className="p-4 rounded-xl border border-border/50 bg-muted/20 space-y-3">
            <div className="flex items-center gap-2">
              ...
              {lang !== 'PT' && <span className="text-[10px] text-muted-foreground">— deixe em branco para usar PT como fallback</span>}
            </div>
            <div className="grid grid-cols-1 gap-3">
              ...
              <div className="space-y-1">
                <label className={labelCls}>Dica Histórica</label>
                <textarea
                  required={lang === 'PT' || lang === 'EN'}
      ...
      <div className="space-y-1">
        ...
        />
        <p className="text-[11px] text-muted-foreground">Suporta Unsplash, Imgur, Wikimedia e Google Drive (o link será convertido automaticamente).</p>
        {formData.imagem_principal_url && isConvertibleUrl(formData.imagem_principal_url) && (
          <p className="text-[11px] text-amber-600 dark:text-amber-400 flex items-center gap-1 mt-0.5">
            ...
            Link do Google Drive detectado — será convertido automaticamente ao publicar.
          </p>
        )}
      </div>
      ...
      >
        {isLoading ? 'Salvando...' : 'Publicar Desafio'}
      </button>
    </form>
  );
}
